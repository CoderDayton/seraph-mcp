...
class SemanticCacheConfig(BaseModel):
    ...
    embedding_api_key: Optional[str] = Field(
        default=None,
        description="API key for embedding provider (if using API)"
    )
    ...
    max_results: int = Field(
        default=10,
        ge=1,
        description="Maximum results to return from similarity search"
    )

    # ChromaDB configuration
    collection_name: str = Field(
        default="seraph_semantic_cache",
    ...
    max_cache_entries: int = Field(
        default=10000,
        ge=1,
        description="Maximum entries before cleanup"
    )
    ...
    cache_embeddings: bool = Field(
        default=True,
        description="Cache generated embeddings in memory"
    )
